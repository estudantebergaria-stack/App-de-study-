...
import { Layout, Zap, Target, BarChart2, Download, Menu, X, CheckSquare, Calendar as CalendarIcon, Loader2, LogOut } from 'lucide-react';
...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        if (user && db) {
          // User is logged in - load from Firestore with real-time sync
          ...
          unsubscribeRef.current = onSnapshot(userDocRef, (docSnap) => {
            if (docSnap.exists()) {
              ...
              setDoc(userDocRef, INITIAL_STATE).catch(console.error);
              setAppDataState(INITIAL_STATE);
            }
          ...
          // User is not logged in - load from IndexedDB
  ...
  const setAppData = useCallback((updater: (prev: AppState) => AppState) => {
    setAppDataState(prev => {
      ...
      // Save to Firestore if user is logged in, otherwise save to IndexedDB
      if (user && db) {
        const userDocRef = doc(db, 'users', user.uid);
...
// AppGate component - checks if user is logged in
const AppGate: React.FC = () => {
  ...
  // If Firebase is not configured, render app directly (local mode only)
  if (!isFirebaseEnabled) {
    return <AppContent />;
  }

  // If Firebase is configured but user is not logged in, show login page
  if (!user) {
    return <LoginPage />;
  }
